...
router.get('/editProfile', async (req, res) => {
    ...
    try{
        ...
        res.render('home/editProfile',{user, userDetail})
    }catch (err){
...
router.use('/profilePhotos', express.static(path.join(__dirname, '..', '..', 'model', 'profilePhotos')));

router.post('/changePhoto/:userID', async (req, res) => {
    if(req.files){
        ...
        // path to profilePhotos dir
        const uploadDir = path.join(__dirname, '..', '..', 'model', 'profilePhotos');
        if (!fs.existsSync(uploadDir)) {
            fs.mkdirSync(uploadDir, { recursive: true });
        }
        // full path for file
        const uploadPath = path.join(uploadDir, filename);
        // mv file to profilePhotos dir
        file.mv(uploadPath, async (err) => {
            ...
            try{
                await UserDetails.findOneAndUpdate({userID: userID}, {profileName: filename})
